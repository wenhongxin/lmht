...
<template>
	<div id="stu_ipad_pay_ok" class="content_sub">
		...
		<a href='/studentfile/stu_ipad_charge_prove' target="_blank" class='common_btn' @click='backstuinfo'>打印收据</a>
	</div>
</template>
...
<script>
	...
	export default {
		...
		beforeCreate() {
			...
			_self.stu_id = common.getS('stuInfo').student_id;
			if(_self.stu_id) {
				liSrv.studentClassList(_self.stu_id, function(res) {
					_self.classlist = res;
				})
			}
